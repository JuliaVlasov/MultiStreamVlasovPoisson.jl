...
function compute_initial_condition(mesh_x::AbstractMesh,grid_v::AbstractGrid,k::Float64,T::Float64,test_case::String)
    ...
    for j in 1:nv
        ...
        for i in 1:nx
            x_i = mesh_x.x[i]
            rho[i, j] = f0(x_i, alpha,k,T,u_ini(x_i,k,test_case)) / mean_f0(alpha,T,mesh_x,test_case)
            u[i, j] = alpha
...
function poisson!(phi::Vector{Float64}, mesh::UniformMesh, rho_tot::Vector{Float64},  ϵ::Float64)

rho_tot_f=fft(rho_tot.-1)
rho_tot_f[1]=0
kkx=mesh.kx
kkx[1]=1
#ff_PB= ( ϵ* ϵ)*kkx .* kkx.+ 1.0  ## This case is for  The Poisson-Boltzmann equation
ff_P = ( ϵ* ϵ)*kkx.* kkx
...
function main()
    ...
    while n * dt <= tfinal
        ...
        xq = [[compute_feet_char(i, dt, mesh_x, view(u_pred, :,j)) for i in 1:nx] for j in 1:nv]
    ...
    plot_f = plot(mesh_x.x, grid_v.v, f_on_grid', st = [:surface],camera = (0,90),xlabel = "x", ylabel="v")
